...
class TransferService {
    public static boolean transfer(Connection conn, String from, String to, int amount) throws SQLException {
        try (PreparedStatement ps1 = conn.prepareStatement("SELECT balance FROM bank_account WHERE owner = ?")) {
            ps1.setString(1, from);
            try (ResultSet rs1 = ps1.executeQuery()) {
                if (rs1.next() == false) {          // ← 改①：原来是 rs.next()，rs 这个变量在这里不存在
        ...
        try {
            try (PreparedStatement ps3 = conn
                    .prepareStatement("UPDATE bank_account SET balance = balance - ? WHERE owner = ?")) {
                ps3.setInt(1, amount);
                ps3.setString(2, from);
                if (ps3.executeUpdate() != 1) {
                    throw new SQLException("扣钱失败");
                }
            }
            try (PreparedStatement ps4 = conn.prepareStatement("UPDATE bank_account SET balance = balance + ? WHERE owner = ?")) {
                ps4.setInt(1, amount);
                ps4.setString(2, to);
